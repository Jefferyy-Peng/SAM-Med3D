...
############################################################ convert picai
if convert_picai:
    root_path = '/home/yxpengcs/Datasets/MRI/picai'
    img_path = os.path.join(root_path, 'Train', 'images')
    output_root = '/home/yxpengcs/PycharmProjects/SAM-Med3D/sam3d_train/medical_data_all/prostate/picai'
    AI_lesion_annotation_path = '/home/yxpengcs/Datasets/MRI/picai/picai_labels/csPCa_lesion_delineations/AI/Bosma22a'
    ...
    for train_case in use_split['train']:
        for i in range(3):
            if os.path.exists(os.path.join(img_path, train_case + f'_000{str(i)}.nii.gz')):
                meta['training'].append({'image': './' + train_case + f'_000{str(i)}.nii.gz', 'label': './' + train_case + f'_000{str(i)}.nii.gz'})
    ...
    for test_case in use_split['val']:
        for i in range(3):
            if os.path.exists(os.path.join(img_path, test_case + f'_000{str(i)}.nii.gz')):
                meta['test'].append({'image': './' + test_case + f'_000{str(i)}.nii.gz', 'label': './' + test_case + f'_000{str(i)}.nii.gz'})
