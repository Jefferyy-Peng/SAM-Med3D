...
class SmallDecoder(nn.Module):
    def __init__(self, full_features, out, args):
        super(SmallDecoder, self).__init__()
        self.up1 = UpBlockSkip(full_features[3] + full_features[2], full_features[2],
                               func='relu', drop=0)
        self.up2 = UpBlockSkip(full_features[2] + full_features[1], full_features[1],
...
class ModelEmb(nn.Module):
    def __init__(self, args, freeze=True):
        ...
        self.args = args
        self.backbone = HarDNet(depth_wise=bool(int(args.depth_wise)), arch=int(args.order), pretrained=False, args=args)
        d, f = self.backbone.full_features, self.backbone.features
        self.decoder = SmallDecoder(d, out=384, args=args)
        if freeze:
            for param in self.backbone.parameters():
    ...
    def forward(self, img, size=None):
        ...
        dense_embeddings = F.interpolate(dense_embeddings, (self.args.img_size//16, self.args.img_size//16, self.args.img_size//16), mode='trilinear', align_corners=True)
